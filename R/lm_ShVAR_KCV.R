                         ...
                         prior_type = c('NCJ', 'CJ'), num_folds = 5, m0 = ncol(Y))
{
  ...
  #                            num_folds = 5)
  # Shrinkage estimation method for VAR models using 
  ...
  if (is.null(lambda_var))
    lambda_var = c(1e-3,  seq(1e-2,1-1e-2,by=0.01), 1-1e-3, 1-1e-5, 1)
  ...
  if (lenLV*lenL*lenD > 1)
  {
    ...
    for (idD in 1:lenD) {
        ...
        for (fold in 1:num_folds) { 
            ...
            XpTrain[,(K-d*p+1):K] <- XpTrain[,(K-d*p+1):K] / rep( sqrt(v1TR), each=length(tidx[[fold]]))
            ...
            if (!is.infinite(dof_curr)) {
            		for (idL in 1:lenL) {
            		    ...
            		    for (idLV in 1:lenLV) {
                			...
                			vhat = (1 - lambda_var_curr) * v1TR  + lambda_var_curr * median(v1TR)
                			scaledPsihat = Psihat
                			scaledPsihat[(K-d*p+1):K,] =  (scaledPsihat[(K-d*p+1):K,] / sqrt(vhat))
                			scaledPsihat =  scaledPsihat * rep(sqrt(vhat), each=K)
                
                			pe_k <- sum((XfValid -  XpValid %*% scaledPsihat)^2) / nrow(XfValid)
