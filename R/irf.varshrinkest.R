...
#'
#' It runs \code{vars::irf()} for an object of class "varshrinkest".
#'
...
irf.varshrinkest <-
  ...
  result <- irf(x, impulse = impulse, response = response, n.ahead = n.ahead,
                ortho = ortho, cumulative = cumulative, boot = boot, ci = ci,
                runs = runs, seed = seed, ...)
  return(result)
}
