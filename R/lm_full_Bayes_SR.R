lm_full_Bayes_SR <- function(Y, X, dof=Inf, burnincycle=1000, mcmccycle=2000) {
  ...
  #         So, p can be estimated by p == K %/% D
  #
  #   dof : If dof = Inf, then we apply multivarate normal distribution, and
  #     do not estimate Q. If dof = 'a finite positive value', then we apply
  #     the multivariate t-distribution with fixed dof, and estimate Q. If
  #     dof = NULL or dof <= 0, we estimate both dof and Q.
  #
  # OUTPUT
  ...
  #             If dof is fixed (given as an argument), then se.param$dof==0.
  ...
  #Gibbs sampler: burn-in cycles & MCMC cycles
    ...
    eW <- eigen(W)
	  ...
	  if (is.null(dof) || !is.infinite(dof))
	  {
	    ...
	    if (is.infinite(w)) {
	      ...
	      #gamma distribution with alpha=(0.5(nu + d)), scale = 1/x
	      Q <- rgamma(N, shape = (w + 0.5 * d), scale = 1) / x
	    }
	  }
		######################################
	...
	mySE.Psi02 <- matrix(sqrt(Psi02SE - Psi02^2 / (mcmccycle - 1)), K, d)
