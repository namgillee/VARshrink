...
{ 
  ...
  v1 = apply(tsDatc, 2, var)  # a row-vector of sample variances
	...
	estimate_lambda_var = FALSE  			#attr(myPsi,"lambda_var.estimated") = FALSE
    ...
    # Compute var_s = var_r + cov_r
		...
		for (k in 1:(lenT-1)) {
			for (k2 in (k+1):lenT) {  # k < k2
				if ( k2-k >= lenT-1 ) {
					cov_r = cov_r + 2 / ((lenT-1)^2) / lenT * sum( DatW[,-(((lenT-(k2-k))+1):lenT)]*DatW[,-(1:(k2-k))] )
				} else {
					cov_r = cov_r + 2 / ((lenT-1)^2) / lenT * sum( rowSums( DatW[,-(((lenT-(k2-k))+1):lenT)]*DatW[,-(1:(k2-k))] ) )
				}
			}
		}
		var_s =  var_r + cov_r
  ...
  if (lenD * lenL > 1)
  {
    ...
    for (fold in seq(1,numAdded,length.out=numAdded)) {
      vidx[[fold+numDeflt]] = idx_s[ (1 + tmpnum + (fold-1)*(bsize+1)):(tmpnum + fold*(bsize+1)) ]; 
    ...
    for (idD in 1:lenD) {
      ...
      for (fold in 1:num_folds) { 
          ...
          for (idL in 1:lenL) {
              lambd2 <- lambda[idL]
              Psihat <- shrinkVARcoef(Y = XfTrain, X = XpTrain, lambda = lambd2, 
  			                          dof = dof_curr, prior_type = prior_type, m0 = m0)
          ...
          lgtheta1 <- log( lambd1*(nrow(XpTrain)-1)/(1-lambd1)/(d*K) )
          eta_bar <- eta_bar + lgtheta1/num_folds    #log inv eta
          #MSE_ave <- MSE_ave + mse1/num_folds#not_used
      ...
      for (fold in 1:num_folds) { 
          ...
          XfValid <- matrix( Y[vidx[[fold]],] , length(vidx[[fold]]) )
	        ...
	        Psihat <- shrinkVARcoef(Y = XfTrain, X = XpTrain, lambda = lambda_curr, 
  			                      dof = dof_curr, prior_type = prior_type, m0 = m0)
