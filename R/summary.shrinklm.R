summary.shrinklm <- function (object, correlation = FALSE, symbolic.cor = FALSE,
                              ...)
{
  z <- object
  p <- z$rank
  ...
  #Qr <- qr.lm(z)
  #n <- NROW(Qr$qr)
  if (is.na(z$df.residual))  #if (is.na(z$df.residual) || n - p != z$df.residual)
  ...
  est <- z$coefficients[p2]  # est <- z$coefficients[Qr$pivot[p1]]
  tval <- est/se
  ans <- z[c("call", "terms", if (!is.null(z$weights)) "weights")]
  ans$residuals <- r
  ...
  ans$aliased <- is.na(coef(z))
  ans$sigma <- sqrt(resvar)
  ans$df <- c(p, rdf, p)  # ans$df <- c(p, rdf, NCOL(Qr$qr))
