...
- \code{lambda, lambda_var}:  shrinkage parameter value(s). Use of this parameter is slightly different for each method. See descriptions in the following subsections for the use of shrinkage parameters in each method.
...
resu_sse <- data.frame(SSE = sapply(resu_estim,
  function(x) calcSSE_Acoef(Acoef.varshrinkest(x), myCoef$A)))
...
for (idnumT in 1:length(NumTimePTS)) {
  ...
  for (r in 1:nrep) {
    ...
    resu_SSE[r, 1, idnumT] <- calcSSE_Acoef(Acoef.varshrinkest(EstimRG), Ad)
    resu_SSE[r, 2, idnumT] <- calcSSE_Acoef(Acoef.varshrinkest(EstimNS), Ad)
    resu_SSE[r, 3, idnumT] <- calcSSE_Acoef(Acoef.varshrinkest(EstimFB), Ad)
    resu_SSE[r, 4, idnumT] <- calcSSE_Acoef(Acoef.varshrinkest(EstimSB), Ad)
  }
